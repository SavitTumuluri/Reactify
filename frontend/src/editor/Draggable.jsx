...
function useRafState(initial,ir) {
  const frame = useRef(null);
  const [state, setState] = ir.useState("pos",initial);
...
export function Draggable({
  ...
  style,
    ...
    const [ir] = useState(() => new DraggableIR())
  const ref = useRef(null);
  const [pos, setPos] = useRafState(initial,ir);
  ...
  const endDrag = () => {
    ...
    console.log(ir.createReactString())
  };
...
export function DraggableCreator(data) {
    let {x,y} = data.pos
    


    return `function DraggableStatic({children,style,className}) {
        const content =
        typeof children === "function"
        ? children({dragging:false, position: {x:${x},y:${y}} })
        : children;
        const wrapperStyle = {
